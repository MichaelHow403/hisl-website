...
export default function ImmersiveGlobePage() {
  ...
  const handleSubmitPrompt = async () => {
    ...
    // Enhanced fallback responses with HISL branding (using as primary for reliability)
    const intelligentResponses = [
      ...
      `HISL Sovereign Response: Your query "${prompt.substring(0, 40)}${prompt.length > 40 ? '...' : ''}" demonstrates our commitment to data sovereignty. The request was processed entirely within our controlled infrastructure, with Huginn managing thought processes and Muninn handling memory operations. Total processing time: ${currentLatency}ms across ${Math.floor(Math.random() * 3) + 2} geographically distributed but jurisdiction-compliant nodes.`
    ];
    
    const responseText = intelligentResponses[Math.floor(Math.random() * intelligentResponses.length)];
